...
module LogRel
  (l : Universe-level) (rec : ∀ {ℓ} {Γ : Con Term ℓ} {l′} {[l′] : Γ ⊩Level l′ ∷Level} → Γ ⊩ [l′] <ᵘ l → LogRelKit)
  where
  ...
  mutual
    ...
    data _⊩ₗ_ (Γ : Con Term ℓ) : Term ℓ → Set a where
      ...
      Idᵣ : ∀ {A} → Γ ⊩ₗId A → Γ ⊩ₗ A
      emb : ∀ {A l′} {[l′] : Γ ⊩Level l′ ∷Level} (l< : Γ ⊩ [l′] <ᵘ l) (let open LogRelKit (rec l<))
            ([A] : Γ ⊩ A) → Γ ⊩ₗ A
