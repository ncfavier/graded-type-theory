...
~-natrec _ x x₁ x₂ (↑ A≡B x₄) =
  ...
  in  ↑ (refl (substType ⊢F ⊢n))
        (natrec-cong x x₁ x₂ k~l′ PE.refl PE.refl)
...
eqRelInstance = record {
  ...
  ≅-Π-cong = λ x x₁ x₂ → liftConv (Π-cong x x₁ x₂ PE.refl PE.refl);
  ≅ₜ-Π-cong = λ x x₁ x₂ → let _ , F∷U , H∷U = syntacticEqTerm (soundnessConv↑Term x₁)
                          ...
                          in  liftConvTerm (univ (Πⱼ F∷U ▹ G∷U) (Πⱼ H∷U ▹ E∷U′)
                                                (Π-cong x F<>H G<>E PE.refl PE.refl));
  ≅-Σ-cong = λ x x₁ x₂ → liftConv (Σ-cong x x₁ x₂ PE.refl);
