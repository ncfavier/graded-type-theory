...
opaque mutual
  ...
  _⊩ᵛᵘ_ : Con Term n → Term n → Set a
  _⊩ᵛᵘ_ {n} Γ l =
    ⊩ᵛ Γ ×
      ∀ {m Δ} {σ₁ σ₂ : Subst m n} → Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ →
        Δ ⊩Level l [ σ₁ ] ≡ l [ σ₂ ] ∷Level

  -- Valid types.

  infix 4 _⊩ᵛ⟨_⟩_

  _⊩ᵛ⟨_⟩_ : Con Term n → Term n → Term n → Set a
  Γ ⊩ᵛ⟨ l ⟩ A = Γ ⊩ᵛ⟨ l ⟩ A ≡ A
  ...
  _⊩ᵛ⟨_⟩_≡_ : Con Term n → Term n → Term n → Term n → Set a
  _⊩ᵛ⟨_⟩_≡_ {n} Γ l A B =
    Γ ⊩ᵛᵘ l ×
      ∀ {m Δ} {σ₁ σ₂ : Subst m n} → Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ →
        Δ ⊩⟨ l [ σ₁ ] ⟩ A [ σ₁ ] ≡ B [ σ₂ ]
  ...
  Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ ∙ A =
    Δ ⊩ˢ tail σ₁ ≡ tail σ₂ ∷ Γ ×
      (∃ λ l → Γ ⊩ᵛ⟨ l ⟩ A ×
        Δ ⊩⟨ l [ tail σ₁ ] ⟩ head σ₁ ≡ head σ₂ ∷ A [ tail σ₁ ])
...
opaque
  ...
  -- Valid level inequality.

  infix 4 _⊩ᵛᵘ_≤_

  _⊩ᵛᵘ_≤_ : Con Term n → Term n → Term n → Set a
  _⊩ᵛᵘ_≤_ {n} Γ l₁ l₂ =
    Γ ⊩ᵛᵘ l₁ × Γ ⊩ᵛᵘ l₂ ×
      ∀ {m Δ} {σ : Subst m n} → Δ ⊩ˢ σ ∷ Γ →
        Δ ⊩ l₁ [ σ ] ≤ l₂ [ σ ] ∷Level
...
opaque
  ...
  -- A characterisation lemma for _⊩ᵛᵘ_.
...
opaque
  ...
  ⊩ˢ∷∙⇔ :
    ...
    (Δ ⊩ˢ tail σ ∷ Γ ×
      (∃ λ l → (Γ ⊩ᵛ⟨ l ⟩ A) ×
        Δ ⊩⟨ l [ tail σ ] ⟩ head σ ∷ A [ tail σ ]))
  ⊩ˢ∷∙⇔ = Σ-cong-⇔ λ _ → Σ-cong-⇔ λ _ → Σ-cong-⇔ λ _ → sym⇔ ⊩∷⇔⊩≡∷
...
opaque
  ...
  -- A substitution lemma for _⊩ᵛᵘ_ and _⊩Level_∷Level.

  ⊩ᵛᵘ→⊩ˢ≡∷→⊩[]≡[] :
    Γ ⊩ᵛᵘ l →
    Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ →
    Δ ⊩Level l [ σ₁ ] ≡ l [ σ₂ ] ∷Level
  ⊩ᵛᵘ→⊩ˢ≡∷→⊩[]≡[] ⊩l σ₁≡σ₂ = ⊩ᵛᵘ⇔ .proj₁ ⊩l .proj₂ σ₁≡σ₂

opaque

  -- Another substitution lemma for _⊩ᵛᵘ_ and _⊩Level_∷Level.

  ⊩ᵛᵘ→⊩ˢ∷→⊩[] :
    Γ ⊩ᵛᵘ l →
    Δ ⊩ˢ σ ∷ Γ →
    Δ ⊩Level l [ σ ] ∷Level
  ⊩ᵛᵘ→⊩ˢ∷→⊩[] ⊩l ⊩σ = wf-⊩Level (⊩ᵛᵘ→⊩ˢ≡∷→⊩[]≡[] ⊩l (refl-⊩ˢ≡∷ ⊩σ)) .proj₁
...
opaque
  ...
  ⊩ᵛ≡→⊩ˢ≡∷→⊩[]≡[] :
    ...
    Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ →
    Δ ⊩⟨ l [ σ₁ ] ⟩ A [ σ₁ ] ≡ B [ σ₂ ]
  ⊩ᵛ≡→⊩ˢ≡∷→⊩[]≡[] A≡B σ₁≡σ₂ = ⊩ᵛ≡⇔ .proj₁ A≡B .proj₂ σ₁≡σ₂
...
opaque
  ...
  sym-⊩ᵛ≡ A≡B =
    ...
    ⊩ᵛ≡⇔ .proj₂
      ( ⊩l
      , λ σ₁≡σ₂ →
        level-conv-⊩≡ (symLevel (⊩ᵛᵘ⇔ .proj₁ ⊩l .proj₂ σ₁≡σ₂))
        $ sym-⊩≡ $ A≡B $ sym-⊩ˢ≡∷ σ₁≡σ₂
      )
...
opaque
  ...
  trans-⊩ˢ≡ {Γ = _ ∙ _} σ₁≡σ₂ σ₂≡σ₃ =
    ...
    ⊩ˢ≡∷∙⇔ .proj₂
      ...
      , l , ⊩A
      , level-conv-⊩≡∷
          (symLevel (⊩ᵛᵘ⇔ .proj₁ (⊩ᵛ⇔ .proj₁ ⊩A .proj₁) .proj₂ σ₁₊≡σ₂₊))
          (trans-⊩≡∷ σ₁₀≡σ₂₀ σ₂₀≡σ₃₀)
      )
...
opaque
  ...
  -- A level well-formedness lemma for _⊩ᵛ⟨_⟩_.

  wfᵘ-⊩ᵛ : Γ ⊩ᵛ⟨ l ⟩ A → Γ ⊩ᵛᵘ l
  wfᵘ-⊩ᵛ ⊩A = ⊩ᵛ⇔ .proj₁ ⊩A .proj₁

opaque

  -- A level well-formedness lemma for _⊩ᵛ⟨_⟩_≡_.

  wfᵘ-⊩ᵛ≡ : Γ ⊩ᵛ⟨ l ⟩ A ≡ B → Γ ⊩ᵛᵘ l
  wfᵘ-⊩ᵛ≡ A≡B = ⊩ᵛ≡⇔ .proj₁ A≡B .proj₁

opaque

  -- A level well-formedness lemma for _⊩ᵛ⟨_⟩_∷_.

  wfᵘ-⊩ᵛ∷ : Γ ⊩ᵛ⟨ l ⟩ t ∷ A → Γ ⊩ᵛᵘ l
  wfᵘ-⊩ᵛ∷ ⊩t = wfᵘ-⊩ᵛ (⊩ᵛ∷⇔ .proj₁ ⊩t .proj₁)

opaque

  -- A level well-formedness lemma for _⊩ᵛ⟨_⟩_≡_∷_.

  wfᵘ-⊩ᵛ≡∷ : Γ ⊩ᵛ⟨ l ⟩ t ≡ u ∷ A → Γ ⊩ᵛᵘ l
  wfᵘ-⊩ᵛ≡∷ t≡u = wfᵘ-⊩ᵛ (⊩ᵛ≡∷⇔ .proj₁ t≡u .proj₁)
    ...
    , (λ (⊩A , _ , A≡B) →
         ...
         ⊩ᵛ≡⇔ .proj₂
           ...
           , λ {_ _} {σ₁ = σ₁} {σ₂ = σ₂} σ₁≡σ₂ →
               ...
               A [ σ₂ ]  ≡⟨ level-conv-⊩≡ (symLevel (⊩ᵛᵘ⇔ .proj₁ ⊩l .proj₂ σ₁≡σ₂)) (A≡B $ wf-⊩ˢ≡∷ σ₁≡σ₂ .proj₂) ⟩⊩∎
               B [ σ₂ ]  ∎
           ))
...
opaque
  ...
  ⊩ˢ∷∙⇔′ :
    Δ ⊩ˢ σ ∷ Γ ∙ A ⇔
    ((∃ λ l → Γ ⊩ᵛ⟨ l ⟩ A) ×
     (∃ λ l → Δ ⊩⟨ l [ tail σ ] ⟩ head σ ∷ A [ tail σ ]) ×
     Δ ⊩ˢ tail σ ∷ Γ)
  ⊩ˢ∷∙⇔′ {Δ} {σ} {Γ} {A} =
    Δ ⊩ˢ σ ∷ Γ ∙ A                                              ⇔⟨ ⊩ˢ∷∙⇔ ⟩
    (Δ ⊩ˢ tail σ ∷ Γ ×
      (∃ λ l → (Γ ⊩ᵛ⟨ l ⟩ A) ×
        Δ ⊩⟨ l [ tail σ ] ⟩ head σ ∷ A [ tail σ ]))
                                                                ⇔⟨ (λ (⊩tail , l , ⊩A , ⊩head) →
                                                                      (l , ⊩A) , (l , ⊩head) , ⊩tail)
                                                                 , (λ ((l₁ , ⊩A) , (_ , ⊩head) , ⊩tail) →
                                                                      ⊩tail , l₁ , ⊩A , level-⊩∷ (⊩ᵛ→⊩ˢ∷→⊩[] ⊩A ⊩tail) ⊩head)
                                                                 ⟩
    (∃ λ l → Γ ⊩ᵛ⟨ l ⟩ A) ×
    (∃ λ l → Δ ⊩⟨ l [ tail σ ] ⟩ head σ ∷ A [ tail σ ]) ×
    Δ ⊩ˢ tail σ ∷ Γ                                             □⇔
...
opaque
  ...
  ⊩ˢ≡∷∙⇔′ :
    Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ ∙ A ⇔
    ((∃ λ l → Γ ⊩ᵛ⟨ l ⟩ A) ×
     (∃ λ l → Δ ⊩⟨ l [ tail σ₁ ] ⟩ head σ₁ ≡ head σ₂ ∷ A [ tail σ₁ ]) ×
     Δ ⊩ˢ tail σ₁ ≡ tail σ₂ ∷ Γ)
  ⊩ˢ≡∷∙⇔′ {Δ} {σ₁} {σ₂} {Γ} {A} =
    Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ ∙ A                                            ⇔⟨ ⊩ˢ≡∷∙⇔ ⟩
    (Δ ⊩ˢ tail σ₁ ≡ tail σ₂ ∷ Γ ×
      (∃ λ l → (Γ ⊩ᵛ⟨ l ⟩ A) ×
        Δ ⊩⟨ l [ tail σ₁ ] ⟩ head σ₁ ≡ head σ₂ ∷ A [ tail σ₁ ]))
                                                                    ⇔⟨ (λ (tail≡tail , l , ⊩A , head≡head) →
                                                                          (l , ⊩A) , (l , head≡head) , tail≡tail)
                                                                     , (λ ((l₁ , ⊩A) , (_ , head≡head) , tail≡tail) →
                                                                            tail≡tail , l₁ , ⊩A
                                                                          , level-⊩≡∷ (⊩ᵛ→⊩ˢ∷→⊩[] ⊩A (wf-⊩ˢ≡∷ tail≡tail .proj₁)) head≡head
                                                                        )
                                                                     ⟩
    (∃ λ l → Γ ⊩ᵛ⟨ l ⟩ A) ×
    (∃ λ l → Δ ⊩⟨ l [ tail σ₁ ] ⟩ head σ₁ ≡ head σ₂ ∷ A [ tail σ₁ ]) ×
...
opaque
  ...
  conv-⊩ˢ≡∷-∙ {Γ} {A} {B} {Δ} {σ₁} {σ₂} A≡B =
    ...
    _                                      →⟨ (λ (σ₁₊≡σ₂₊ , _ , ⊩A , σ₁₀≡σ₂₀) →
                            σ₁₊≡σ₂₊ , _ , ⊩B , conv-⊩≡∷ (A≡B $ wf-⊩ˢ≡∷ σ₁₊≡σ₂₊ .proj₁) σ₁₀≡σ₂₀) ⟩
    _                                      ⇔˘⟨ ⊩ˢ≡∷∙⇔ ⟩→
    ...
    case ⊩ᵛ≡⇔′ .proj₁ B₁≡B₂ of λ
      (⊩B₁ , ⊩B₂ , B₁≡B₂) →
        ...
        ⊩ˢ≡∷∙⇔ .proj₂
          ( conv-⊩ˢ≡∷-∙ A₁≡A₂ σ₁₊≡σ₂₊ , _ , ⊩B₂
          , conv-⊩≡∷
            (B₁≡B₂ (conv-⊩ˢ∷-∙ A₁≡A₂ (wf-⊩ˢ≡∷ σ₁₊≡σ₂₊ .proj₁)))
            σ₁₀≡σ₂₀
          )

opaque

  -- Conversion for the context for _⊩ᵛᵘ_.

  conv-∙-⊩ᵛᵘ :
    Γ ⊩ᵛ⟨ l′ ⟩ A ≡ B →
    Γ ∙ A ⊩ᵛᵘ l →
    Γ ∙ B ⊩ᵛᵘ l
  conv-∙-⊩ᵛᵘ {Γ} {A} {B} {l} A≡B ⊩l =
    case ⊩ᵛ≡⇔′ .proj₁ A≡B of λ
      (⊩A , ⊩B , A≡B′) →
    case ⊩ᵛᵘ⇔ .proj₁ ⊩l of
      λ (⊩Γ , ⊩l′) →
  ...
  conv-∙-⊩ᵛ {Γ} {A} {B} {l} {C} A≡B ⊩C =
    ...
    ⊩ᵛ⇔ .proj₂
      ...
      , λ {_} {Δ = Δ} {σ₁ = σ₁} {σ₂ = σ₂} σ₁≡σ₂ →
          ...
          $ ⊩ˢ≡∷∙⇔ .proj₂
            ( σ₁₊≡σ₂₊ , _ , ⊩A
            , conv-⊩≡∷ (sym-⊩≡ $ A≡B′ $ wf-⊩ˢ≡∷ σ₁₊≡σ₂₊ .proj₁) σ₁₀≡σ₂₀
            )
      )

opaque

  -- Another kind of conversion for the context for _⊩ᵛᵘ_.
...
opaque
  ...
  cast-⊩ˢ∷ {Γ = _ ∙ A} σ₁≡σ₂ ⊩σ₁ =
    ...
    ⊩ˢ∷∙⇔ .proj₂
      ( cast-⊩ˢ∷ σ₁₊≡σ₂₊ ⊩σ₁₊
      , _ , ⊩A
      , PE.subst₃ (_ ⊩⟨_⟩_∷_) (substVar-to-subst σ₁₊≡σ₂₊ l) (σ₁≡σ₂ x0) (substVar-to-subst σ₁₊≡σ₂₊ A) ⊩σ₁₀
      )
...
opaque
  ...
  ⊩ˢ≡∷-•ₛ {Γ = _ ∙ A} ρ⊇ σ₁≡σ₂ =
    ...
    ⊩ˢ≡∷∙⇔ .proj₂
      ( ⊩ˢ≡∷-•ₛ ρ⊇ σ₁₊≡σ₂₊
      , _ , ⊩A
      , PE.subst₂ (_ ⊩⟨_⟩ _ ≡ _ ∷_) (wk-subst l) (wk-subst A)
          (wk-⊩≡∷ ρ⊇ σ₁₀≡σ₂₀)
      )
...
opaque
  ...
  ⊩ˢ≡∷-ₛ• (TW.step ρ⊇) ⊩Γ σ₁≡σ₂ =
    case ⊩ˢ≡∷∙⇔ .proj₁ σ₁≡σ₂ of λ
      (tail≡tail , _ , ⊩A , head≡head) →
    ⊩ˢ≡∷-ₛ• ρ⊇ ⊩Γ tail≡tail
  ⊩ˢ≡∷-ₛ• {ρ} (TW.lift {A} ρ⊇) ⊩Γ∙A σ₁≡σ₂ =
    case wf-⊩ᵛ-∙ ⊩Γ∙A of λ
      (l′ , ⊩A) →
    case ⊩ˢ≡∷∙⇔ .proj₁ σ₁≡σ₂ of λ
      (tail≡tail , l , _ , head≡head) →
    case ⊩ˢ≡∷-ₛ• ρ⊇ (wf-⊩ᵛ ⊩A) tail≡tail of λ
      wk-tail≡wk-tail →
    ⊩ˢ≡∷∙⇔ .proj₂
      ( wk-tail≡wk-tail
      , l′
      , ⊩A
      , level-⊩≡∷ (⊩ᵛ→⊩ˢ∷→⊩[] ⊩A (wf-⊩ˢ≡∷ wk-tail≡wk-tail .proj₁))
          (PE.subst (_ ⊩⟨ _ ⟩ _ ≡ _ ∷_) (subst-wk A) head≡head)
      )
...
opaque
  ...
  ⊩ˢ≡∷-sgSubst {l} ⊩A t≡u =
    ⊩ˢ≡∷∙⇔′ .proj₂
      ( (_ , ⊩A)
      , (l , PE.subst₂ (_ ⊩⟨_⟩ _ ≡ _ ∷_) (PE.sym $ subst-id _) (PE.sym $ subst-id _) t≡u)
...
opaque
  ...
  ⊩ᵛ≡→⊩≡ {Γ} {l} {A} {B} =
    ...
    (Γ ⊩ᵛ⟨ l ⟩ B) ×
    (∀ {m} {Δ : Con Term m} {σ} →
     Δ ⊩ˢ σ ∷ Γ → Δ ⊩⟨ l [ σ ] ⟩ A [ σ ] ≡ B [ σ ])  →⟨ (λ (⊩A , _ , A≡B) → A≡B $ ⊩ˢ∷-idSubst $ wf-⊩ᵛ ⊩A) ⟩

    Γ ⊩⟨ l [ idSubst ] ⟩ A [ idSubst ] ≡ B [ idSubst ]     ≡⟨ PE.cong₃ (_ ⊩⟨_⟩_≡_) (subst-id _) (subst-id _) (subst-id _) ⟩→
...
opaque
  ...
  ⊩ᵛ≡∷→⊩≡∷ {Γ} {l} {t} {u} {A} =
    ...
    (Γ ⊩ᵛ⟨ l ⟩ u ∷ A) ×
    (∀ {m} {Δ : Con Term m} {σ} →
     Δ ⊩ˢ σ ∷ Γ → Δ ⊩⟨ l [ σ ] ⟩ t [ σ ] ≡ u [ σ ] ∷ A [ σ ])     →⟨ (λ (⊩t , _ , t≡u) → t≡u $ ⊩ˢ∷-idSubst $ wf-⊩ᵛ $ wf-⊩ᵛ∷ ⊩t) ⟩

    Γ ⊩⟨ l [ idSubst ] ⟩ t [ idSubst ] ≡ u [ idSubst ] ∷ A [ idSubst ]  ≡⟨ PE.cong₄ (_ ⊩⟨_⟩_≡_∷_)
                                                                 (subst-id _) (subst-id _) (subst-id _) (subst-id _) ⟩→
...
opaque
  ...
  wk-⊩ᵛᵘ {ρ} {l} ρ⊇ ⊩Δ ⊩l =
    ...
    ⊩ᵛᵘ⇔ .proj₂
      ( ⊩Δ
      , λ {_ _} {σ₁} {σ₂} σ₁≡σ₂ →
          wk ρ l [ σ₁ ] ≡⟨ subst-wk l ⟩⊩Level≡
          l [ σ₁ ₛ• ρ ] ≡⟨ ⊩ᵛᵘ⇔ .proj₁ ⊩l .proj₂ (⊩ˢ≡∷-ₛ• ρ⊇ ⊩Γ σ₁≡σ₂) ⟩⊩Level∎≡
          l [ σ₂ ₛ• ρ ] ≡˘⟨ subst-wk l ⟩
          wk ρ l [ σ₂ ] ∎
      )
...
opaque
  ...
  -- Single-step weakening for _⊩ᵛᵘ_.

  wk1-⊩ᵛᵘ : Γ ⊩ᵛ⟨ l′ ⟩ A → Γ ⊩ᵛᵘ l → Γ ∙ A ⊩ᵛᵘ wk1 l
  wk1-⊩ᵛᵘ ⊩A ⊩l = wk-⊩ᵛᵘ (TW.step TW.id) (⊩ᵛ-∙-intro ⊩A) ⊩l
...
opaque
  ...
  wk-⊩ᵛ≡ :
    ρ ∷ Δ ⊇ Γ → ⊩ᵛ Δ → Γ ⊩ᵛ⟨ l ⟩ A ≡ B → Δ ⊩ᵛ⟨ wk ρ l ⟩ wk ρ A ≡ wk ρ B
  wk-⊩ᵛ≡ {ρ} {l} {A} {B} ρ⊇ ⊩Δ A≡B =
    case ⊩ᵛ≡⇔ .proj₁ A≡B of λ
      (⊩l , A≡B) →
    ⊩ᵛ≡⇔ .proj₂
      ( wk-⊩ᵛᵘ ρ⊇ ⊩Δ ⊩l
      , λ {_ _} {σ₁ = σ₁} {σ₂ = σ₂} σ₁≡σ₂ →
          wk ρ A [ σ₁ ]  ≡⟨ subst-wk A ⟩⊩≡
          A [ σ₁ ₛ• ρ ]  ≡⟨ PE.subst (_ ⊩⟨_⟩ _ ≡ _) (PE.sym (subst-wk l)) (A≡B $ ⊩ˢ≡∷-ₛ• ρ⊇ (⊩ᵛᵘ⇔ .proj₁ ⊩l .proj₁) σ₁≡σ₂) ⟩⊩∎≡
          B [ σ₂ ₛ• ρ ]  ≡˘⟨ subst-wk B ⟩
          wk ρ B [ σ₂ ]  ∎
      )
...
opaque
  ...
  wk1-⊩ᵛ≡ : Γ ⊩ᵛ⟨ l′ ⟩ C → Γ ⊩ᵛ⟨ l ⟩ A ≡ B → Γ ∙ C ⊩ᵛ⟨ wk1 l ⟩ wk1 A ≡ wk1 B
  wk1-⊩ᵛ≡ ⊩C =
...
opaque
  ...
  wk-⊩ᵛ : ρ ∷ Δ ⊇ Γ → ⊩ᵛ Δ → Γ ⊩ᵛ⟨ l ⟩ A → Δ ⊩ᵛ⟨ wk ρ l ⟩ wk ρ A
  wk-⊩ᵛ ρ⊇ ⊩Δ =
...
opaque
  ...
  wk1-⊩ᵛ : Γ ⊩ᵛ⟨ l′ ⟩ B → Γ ⊩ᵛ⟨ l ⟩ A → Γ ∙ B ⊩ᵛ⟨ wk1 l ⟩ wk1 A
  wk1-⊩ᵛ ⊩B =
...
opaque
  ...
  wk-⊩ᵛ≡∷ :
    ...
    Δ ⊩ᵛ⟨ wk ρ l ⟩ wk ρ t ≡ wk ρ u ∷ wk ρ A
  wk-⊩ᵛ≡∷ {ρ} {l} {t} {u} {A} ρ⊇ ⊩Δ t≡u =
    ...
    ⊩ᵛ≡∷⇔ .proj₂
      ...
      , λ {_ _} {σ₁ = σ₁} {σ₂ = σ₂} σ₁≡σ₂ →
          wk ρ t [ σ₁ ] ∷ wk ρ A [ σ₁ ]  ≡⟨ subst-wk t ⟩⊩∷∷≡
                                          ⟨ subst-wk A ⟩⊩∷≡
          t [ σ₁ ₛ• ρ ] ∷ A [ σ₁ ₛ• ρ ]  ≡⟨ PE.subst (_ ⊩⟨_⟩ _ ≡ _ ∷ _) (PE.sym (subst-wk l)) (⊩ᵛ≡∷→⊩ˢ≡∷→⊩[]≡[]∷ t≡u $
                                            ⊩ˢ≡∷-ₛ• ρ⊇ (wf-⊩ᵛ ⊩A) σ₁≡σ₂) ⟩⊩∷∎∷≡
          u [ σ₂ ₛ• ρ ]                  ≡˘⟨ subst-wk u ⟩
          wk ρ u [ σ₂ ]                  ∎
      )
...
opaque
  ...
  wk-⊩ᵛ∷ :
    ρ ∷ Δ ⊇ Γ → ⊩ᵛ Δ → Γ ⊩ᵛ⟨ l ⟩ t ∷ A → Δ ⊩ᵛ⟨ wk ρ l ⟩ wk ρ t ∷ wk ρ A
  wk-⊩ᵛ∷ ρ⊇ ⊩Δ =
    ⊩ᵛ∷⇔⊩ᵛ≡∷ .proj₂ ∘→ wk-⊩ᵛ≡∷ ρ⊇ ⊩Δ ∘→ ⊩ᵛ∷⇔⊩ᵛ≡∷ .proj₁
...
opaque
  ...
  wk1-⊩ᵛ∷ : Γ ⊩ᵛ⟨ l′ ⟩ B → Γ ⊩ᵛ⟨ l ⟩ t ∷ A → Γ ∙ B ⊩ᵛ⟨ wk1 l ⟩ wk1 t ∷ wk1 A
  wk1-⊩ᵛ∷ ⊩B =
    wk-⊩ᵛ∷ (TW.step TW.id) (⊩ᵛ-∙-intro ⊩B)
...
opaque
  ...
  -- A substitution lemma for _⊩ᵛᵘ_.
  ...
  ⊩ᵛᵘ→⊩ᵛ∷→⊩ᵛᵘ[]₀≡[]₀ {l} ⊩l ⊩t =
    ...
    ⊩ᵛᵘ⇔ .proj₂
      ( wf-⊩ᵛ ⊩A
      , λ σ₁≡σ₂ →
          PE.subst₂ (_ ⊩Level_≡_∷Level) (substConsId l) (substConsId l) $
          ⊩ᵛᵘ⇔ .proj₁ ⊩l .proj₂ $
          ⊩ˢ≡∷∙⇔ .proj₂ (σ₁≡σ₂ , _ , ⊩A , ⊩t′ σ₁≡σ₂)
      )
...
opaque
  ...
  ⊩ᵛ≡→⊩ᵛ≡∷→⊩ᵛ[]₀≡[]₀ {l} {B} {C} B≡C t≡u =
    case ⊩ᵛ≡∷⇔ .proj₁ t≡u of λ
      (⊩A , t≡u′) →
    ⊩ᵛ≡⇔ .proj₂
      ( ⊩ᵛᵘ→⊩ᵛ∷→⊩ᵛᵘ[]₀≡[]₀ (⊩ᵛ≡⇔ .proj₁ B≡C .proj₁) (wf-⊩ᵛ≡∷ t≡u .proj₁)
      , λ σ₁≡σ₂ →
          PE.subst₃ (_ ⊩⟨_⟩_≡_) (substConsId l) (substConsId B) (substConsId C) $
          ⊩ᵛ≡→⊩ˢ≡∷→⊩[]≡[] B≡C $
          ⊩ˢ≡∷∙⇔ .proj₂ (σ₁≡σ₂ , _ , ⊩A , t≡u′ σ₁≡σ₂)
      )
...
opaque
  ...
  ⊩ᵛ∷→⊩ᵛ∷→⊩ᵛ[]₀∷ {l} {t} {B} {l′} {u} ⊩t ⊩u =
    ...
    ⊩ᵛ∷⇔ .proj₂
      ...
      , λ σ₁≡σ₂ →
          PE.subst₄ (_ ⊩⟨_⟩_≡_∷_)
            (substConsId l) (substConsId t) (substConsId t) (substConsId B) $
          t≡t $
          ⊩ˢ≡∷∙⇔′ .proj₂
            ( wf-∙-⊩ᵛ ⊩B
            , (l′ , ⊩ᵛ∷⇔ .proj₁ ⊩u .proj₂ σ₁≡σ₂)
...
opaque
  ...
  ⊩ᵛᵘ→⊩ᵛ∷→⊩ᵛ∷→⊩ᵛᵘ[]₁₀ {B} {l} {l″} {l‴} ⊩l ⊩t ⊩u =
    ...
    ⊩ᵛᵘ⇔ .proj₂
      ( wf-⊩ᵛ ⊩A
      , λ σ₁≡σ₂ →
          PE.subst₂ (_ ⊩Level_≡_∷Level) (PE.sym $ [,]-[]-fusion l) (PE.sym $ [,]-[]-fusion l) $
          ⊩l′ $
          ⊩ˢ≡∷∙⇔′ .proj₂
            ( (_ , ⊩B)
            , ( wk1 l‴
              , PE.subst₂ (_ ⊩⟨_⟩ _ ≡ _ ∷_) (PE.sym $ wk1-tail l‴) (PE.sym $ substConsId B)
                  (⊩ᵛ≡∷→⊩ˢ≡∷→⊩[]≡[]∷ (refl-⊩ᵛ≡∷ ⊩u) σ₁≡σ₂)
              )
            , ⊩ˢ≡∷∙⇔′ .proj₂
                ( (_ , ⊩A)
...
opaque
  ...
  ⊩ᵛ≡→⊩ᵛ≡∷→⊩ᵛ≡∷→⊩ᵛ[]₁₀≡[]₁₀ :
    ...
    Γ ⊩ᵛ⟨ l″ ⟩ t₁ ≡ t₂ ∷ A →
    Γ ⊩ᵛ⟨ l‴ ⟩ u₁ ≡ u₂ ∷ B [ t₁ ]₀ →
    Γ ⊩ᵛ⟨ l [ t₁ , u₁ ]₁₀ ⟩ C₁ [ t₁ , u₁ ]₁₀ ≡ C₂ [ t₂ , u₂ ]₁₀
  ⊩ᵛ≡→⊩ᵛ≡∷→⊩ᵛ≡∷→⊩ᵛ[]₁₀≡[]₁₀ {B} {l} {C₁} {C₂} {l″} {l‴} C₁≡C₂ t₁≡t₂ u₁≡u₂ =
    ...
    case wf-∙-⊩ᵛ ⊩B of λ
      (_ , ⊩A) →
    ⊩ᵛ≡⇔ .proj₂
      ( ⊩ᵛᵘ→⊩ᵛ∷→⊩ᵛ∷→⊩ᵛᵘ[]₁₀ ⊩l (wf-⊩ᵛ≡∷ t₁≡t₂ .proj₁) (wf-⊩ᵛ≡∷ u₁≡u₂ .proj₁)
      , λ σ₁≡σ₂ →
          PE.subst₃ (_ ⊩⟨_⟩_≡_)
            (PE.sym $ [,]-[]-fusion l) (PE.sym $ [,]-[]-fusion C₁) (PE.sym $ [,]-[]-fusion C₂) $
          C₁≡C₂ $
          ⊩ˢ≡∷∙⇔′ .proj₂
            ( (_ , ⊩B)
            , ( wk1 l‴
...
opaque
  ...
  ⊩ᵛ≡∷→⊩ᵛ≡∷→⊩ᵛ≡∷→⊩ᵛ[]₁₀≡[]₁₀∷ :
    ...
    Γ ⊩ᵛ⟨ l″ ⟩ u₁ ≡ u₂ ∷ A →
    Γ ⊩ᵛ⟨ l‴ ⟩ v₁ ≡ v₂ ∷ B [ u₁ ]₀ →
    Γ ⊩ᵛ⟨ l [ u₁ , v₁ ]₁₀ ⟩ t₁ [ u₁ , v₁ ]₁₀ ≡ t₂ [ u₂ , v₂ ]₁₀ ∷ C [ u₁ , v₁ ]₁₀
  ⊩ᵛ≡∷→⊩ᵛ≡∷→⊩ᵛ≡∷→⊩ᵛ[]₁₀≡[]₁₀∷ {B} {l} {t₁} {t₂} {C} {l″} {l‴} t₁≡t₂ u₁≡u₂ v₁≡v₂ =
    ...
    ⊩ᵛ≡∷⇔ .proj₂
      ...
      , λ σ₁≡σ₂ →
          PE.subst₄ (_ ⊩⟨_⟩_≡_∷_) (PE.sym $ [,]-[]-fusion l) (PE.sym $ [,]-[]-fusion t₁)
            (PE.sym $ [,]-[]-fusion t₂) (PE.sym $ [,]-[]-fusion C) $
          ⊩ᵛ≡∷→⊩ˢ≡∷→⊩[]≡[]∷ t₁≡t₂ $
          ⊩ˢ≡∷∙⇔′ .proj₂
            ( (_ , ⊩B)
            , ( wk1 l‴
              , (PE.subst₂ (_ ⊩⟨_⟩ _ ≡ _ ∷_) (PE.sym $ wk1-tail l‴)
...
opaque
  ...
  ⊩ᵛᵘ→⊩ᵛ∷→⊩ᵛᵘ[]↑² {A} {l} {l′} ⊩l ⊩t =
    ...
    ⊩ᵛᵘ⇔ .proj₂
      ( wf-⊩ᵛ (wf-⊩ᵛ∷ ⊩t)
      , λ σ₁≡σ₂ →
          PE.subst₂ (_ ⊩Level_≡_∷Level) (substComp↑² l _) (substComp↑² l _) $
          ⊩l′ $
          ⊩ˢ≡∷∙⇔′ .proj₂
            ( (_ , ⊩A)
            , (l′ , PE.subst₂ (_ ⊩⟨_⟩ _ ≡ _ ∷_) (wk2-tail l′) (wk2-tail A)
                (⊩ᵛ∷⇔ .proj₁ ⊩t .proj₂ σ₁≡σ₂))
...
opaque
  ...
  ⊩ᵛ≡→⊩ᵛ∷→⊩ᵛ[]↑²≡[]↑² :
    ...
    Γ ∙ B ∙ C ⊩ᵛ⟨ wk2 l′ ⟩ t ∷ wk2 A →
    Γ ∙ B ∙ C ⊩ᵛ⟨ l [ t ]↑² ⟩ D [ t ]↑² ≡ E [ t ]↑²
  ⊩ᵛ≡→⊩ᵛ∷→⊩ᵛ[]↑²≡[]↑² {A} {l} {D} {E} {l′} D≡E ⊩t =
    case ⊩ᵛ≡⇔ .proj₁ D≡E of λ
      (⊩l , D≡E) →
    ⊩ᵛ≡⇔ .proj₂
      ( ⊩ᵛᵘ→⊩ᵛ∷→⊩ᵛᵘ[]↑² ⊩l ⊩t
      , λ σ₁≡σ₂ →
          PE.subst₃ (_ ⊩⟨_⟩_≡_) (substComp↑² l _) (substComp↑² D _) (substComp↑² E _) $
          D≡E $
          ⊩ˢ≡∷∙⇔′ .proj₂
            ( wf-∙-⊩ᵛᵘ ⊩l
            , ( l′
              , PE.subst₂ (_ ⊩⟨_⟩ _ ≡ _ ∷_) (wk2-tail l′) (wk2-tail A)
                  (⊩ᵛ∷⇔ .proj₁ ⊩t .proj₂ σ₁≡σ₂)
              )
            , ⊩ˢ≡∷∙⇔ .proj₁ (⊩ˢ≡∷∙⇔ .proj₁ σ₁≡σ₂ .proj₁) .proj₁
            )
...
opaque
  ...
  ⊩ᵛ≡∷→⊩ᵛ∷→⊩ᵛ[]↑²≡[]↑²∷ :
    Γ ∙ A ⊩ᵛ⟨ l ⟩ t ≡ u ∷ D →
    Γ ∙ B ∙ C ⊩ᵛ⟨ wk2 l′ ⟩ v ∷ wk2 A →
    Γ ∙ B ∙ C ⊩ᵛ⟨ l [ v ]↑² ⟩ t [ v ]↑² ≡ u [ v ]↑² ∷ D [ v ]↑²
  ⊩ᵛ≡∷→⊩ᵛ∷→⊩ᵛ[]↑²≡[]↑²∷ {A} {l} {t} {u} {D} {l′} t≡u ⊩v =
    case wf-⊩ᵛ∷ (wf-⊩ᵛ≡∷ t≡u .proj₁) of λ
      ⊩D →
    ⊩ᵛ≡∷⇔ .proj₂
      ( ⊩ᵛ→⊩ᵛ∷→⊩ᵛ[]↑² ⊩D ⊩v
      , λ σ₁≡σ₂ →
          PE.subst₄ (_ ⊩⟨_⟩_≡_∷_) (substComp↑² l _) (substComp↑² t _) (substComp↑² u _)
            (substComp↑² D _) $
          ⊩ᵛ≡∷→⊩ˢ≡∷→⊩[]≡[]∷ t≡u $
          ⊩ˢ≡∷∙⇔′ .proj₂
            ( wf-∙-⊩ᵛ ⊩D
...
opaque
  ...
  ⊩ᵛ≡→⊩ˢ≡∷→⊩≡∷→⊩[,]≡[,] :
    ...
    Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ →
    Δ ⊩⟨ l′ [ σ₁ ] ⟩ t₁ ≡ t₂ ∷ A [ σ₁ ] →
    Δ ⊩⟨ l [ consSubst σ₁ t₁ ] ⟩ B₁ [ consSubst σ₁ t₁ ] ≡ B₂ [ consSubst σ₂ t₂ ]
  ⊩ᵛ≡→⊩ˢ≡∷→⊩≡∷→⊩[,]≡[,] {l′} B₁≡B₂ σ₁≡σ₂ t₁≡t₂ =
    ⊩ᵛ≡→⊩ˢ≡∷→⊩[]≡[] B₁≡B₂ $
    ⊩ˢ≡∷∙⇔′ .proj₂ (wf-∙-⊩ᵛ (wf-⊩ᵛ≡ B₁≡B₂ .proj₁) , (l′ , t₁≡t₂) , σ₁≡σ₂)
...
opaque
  ...
  ⊩ᵛ→⊩ˢ∷→⊩∷→⊩[,] :
    ...
    Δ ⊩ˢ σ ∷ Γ →
    Δ ⊩⟨ l′ [ σ ] ⟩ t ∷ A [ σ ] →
    Δ ⊩⟨ l [ consSubst σ t ] ⟩ B [ consSubst σ t ]
  ⊩ᵛ→⊩ˢ∷→⊩∷→⊩[,] {l′} ⊩B ⊩σ ⊩t =
    ⊩⇔⊩≡ .proj₂ $
    ⊩ᵛ≡→⊩ˢ≡∷→⊩≡∷→⊩[,]≡[,] {l′ = l′} (refl-⊩ᵛ≡ ⊩B) (refl-⊩ˢ≡∷ ⊩σ) (refl-⊩≡∷ ⊩t)
...
opaque
  ...
  ⊩ᵛ≡∷→⊩ˢ≡∷→⊩≡∷→⊩[,]≡[,]∷ :
    ...
    Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ →
    Δ ⊩⟨ l′ [ σ₁ ] ⟩ u₁ ≡ u₂ ∷ A [ σ₁ ] →
    Δ ⊩⟨ l [ consSubst σ₁ u₁ ] ⟩ t₁ [ consSubst σ₁ u₁ ] ≡ t₂ [ consSubst σ₂ u₂ ] ∷
      B [ consSubst σ₁ u₁ ]
  ⊩ᵛ≡∷→⊩ˢ≡∷→⊩≡∷→⊩[,]≡[,]∷ {l′} t₁≡t₂ σ₁≡σ₂ u₁≡u₂ =
    ⊩ᵛ≡∷→⊩ˢ≡∷→⊩[]≡[]∷ t₁≡t₂ $
    ⊩ˢ≡∷∙⇔′ .proj₂
      (wf-∙-⊩ᵛ (wf-⊩ᵛ∷ $ wf-⊩ᵛ≡∷ t₁≡t₂ .proj₁) , (l′ , u₁≡u₂) , σ₁≡σ₂)
...
opaque
  ...
  ⊩ᵛ∷→⊩ˢ∷→⊩∷→⊩[,]∷ :
    ...
    Δ ⊩ˢ σ ∷ Γ →
    Δ ⊩⟨ l′ [ σ ]  ⟩ u ∷ A [ σ ] →
    Δ ⊩⟨ l [ consSubst σ u ] ⟩ t [ consSubst σ u ] ∷ B [ consSubst σ u ]
  ⊩ᵛ∷→⊩ˢ∷→⊩∷→⊩[,]∷ {l′} ⊩t ⊩σ ⊩u =
    ⊩∷⇔⊩≡∷ .proj₂ $
    ⊩ᵛ≡∷→⊩ˢ≡∷→⊩≡∷→⊩[,]≡[,]∷ {l′ = l′} (refl-⊩ᵛ≡∷ ⊩t) (refl-⊩ˢ≡∷ ⊩σ) (refl-⊩≡∷ ⊩u)
...
opaque
  ...
  ⊩ᵛ≡→⊩ˢ≡∷→⊩≡∷→⊩[⇑][]₀≡[⇑][]₀ :
    ...
    Δ ⊩⟨ l′ [ σ₁ ] ⟩ t₁ ≡ t₂ ∷ A [ σ₁ ] →
    Δ ⊩⟨ l [ σ₁ ⇑ ] [ t₁ ]₀ ⟩ B₁ [ σ₁ ⇑ ] [ t₁ ]₀ ≡ B₂ [ σ₂ ⇑ ] [ t₂ ]₀
  ⊩ᵛ≡→⊩ˢ≡∷→⊩≡∷→⊩[⇑][]₀≡[⇑][]₀ {l} {B₁} {B₂} {l′} B₁≡B₂ σ₁≡σ₂ t₁≡t₂ =
    PE.subst₃ (_ ⊩⟨_⟩_≡_)
      (PE.sym $ singleSubstComp _ _ l)
...
opaque
  ...
  ⊩ᵛ≡→⊩≡∷→⊩ˢ≡∷→⊩[]₀[]≡[]₀[] :
    ...
    Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ →
    Δ ⊩⟨ l [ t₁ ]₀ [ σ₁ ] ⟩ B₁ [ t₁ ]₀ [ σ₁ ] ≡ B₂ [ t₂ ]₀ [ σ₂ ]
  ⊩ᵛ≡→⊩≡∷→⊩ˢ≡∷→⊩[]₀[]≡[]₀[] {l} {B₁} {B₂} {l′} B₁≡B₂ t₁[σ₁]≡t₂[σ₂] σ₁≡σ₂ =
    PE.subst₃ (_ ⊩⟨_⟩_≡_)
      (PE.sym $ singleSubstLift l _)
      (PE.sym $ singleSubstLift B₁ _)
      (PE.sym $ singleSubstLift B₂ _) $
    ⊩ᵛ≡→⊩ˢ≡∷→⊩≡∷→⊩[⇑][]₀≡[⇑][]₀ {l′ = l′} B₁≡B₂ σ₁≡σ₂ t₁[σ₁]≡t₂[σ₂]
...
opaque
  ...
  ⊩ᵛ≡∷→⊩≡∷→⊩ˢ≡∷→⊩[]₀[]≡[]₀[]∷ :
    ...
    Δ ⊩⟨ l′ [ σ₁ ] ⟩ u₁ [ σ₁ ] ≡ u₂ [ σ₂ ] ∷ A [ σ₁ ] →
    Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ →
    Δ ⊩⟨ l [ u₁ ]₀ [ σ₁ ] ⟩ t₁ [ u₁ ]₀ [ σ₁ ] ≡ t₂ [ u₂ ]₀ [ σ₂ ] ∷ B [ u₁ ]₀ [ σ₁ ]
  ⊩ᵛ≡∷→⊩≡∷→⊩ˢ≡∷→⊩[]₀[]≡[]₀[]∷ {l} {t₁} {t₂} {B} {l′} t₁≡t₂ σ₁≡σ₂ u₁[σ₁]≡u₂[σ₂] =
    PE.subst₄ (_ ⊩⟨_⟩_≡_∷_) (PE.sym $ singleSubstLift l _) (PE.sym $ singleSubstLift t₁ _)
      (PE.sym $ singleSubstLift t₂ _) (PE.sym $ singleSubstLift B _) $
    ⊩ᵛ≡∷→⊩ˢ≡∷→⊩≡∷→⊩[⇑][]₀≡[⇑][]₀∷ {l′ = l′} t₁≡t₂ u₁[σ₁]≡u₂[σ₂] σ₁≡σ₂
...
opaque
  ...
  ⊩ᵛ≡→⊩ˢ≡∷→⊩≡∷→⊩≡∷→⊩[⇑⇑][]₁₀≡[⇑⇑][]₁₀ :
    ...
    Δ ⊩⟨ l′ [ σ₁ ] ⟩ t₁ ≡ t₂ ∷ A [ σ₁ ] →
    Δ ⊩⟨ l″ [ σ₁ ⇑ ] [ t₁ ]₀ ⟩ u₁ ≡ u₂ ∷ B [ σ₁ ⇑ ] [ t₁ ]₀ →
    Δ ⊩⟨ l [ σ₁ ⇑ ⇑ ] [ t₁ , u₁ ]₁₀ ⟩ C₁ [ σ₁ ⇑ ⇑ ] [ t₁ , u₁ ]₁₀ ≡ C₂ [ σ₂ ⇑ ⇑ ] [ t₂ , u₂ ]₁₀
  ⊩ᵛ≡→⊩ˢ≡∷→⊩≡∷→⊩≡∷→⊩[⇑⇑][]₁₀≡[⇑⇑][]₁₀
    {B} {l} {C₁} {C₂} {l′} {l″} C₁≡C₂ σ₁≡σ₂ t₁≡t₂ u₁≡u₂ =
    PE.subst₃ (_ ⊩⟨_⟩_≡_)
      (PE.sym $ doubleSubstComp l _ _ _)
    ...
    PE.subst₂ (_ ⊩⟨_⟩ _ ≡ _ ∷_) (singleSubstComp _ _ l″) (singleSubstComp _ _ B) u₁≡u₂
...
opaque
  ...
  ⊩ᵛ≡∷→⊩ˢ≡∷→⊩≡∷→⊩≡∷→⊩[⇑⇑][]₁₀≡[⇑⇑][]₁₀∷ :
    ...
    Δ ⊩⟨ l′ [ σ₁ ] ⟩ u₁ ≡ u₂ ∷ A [ σ₁ ] →
    Δ ⊩⟨ l″ [ σ₁ ⇑ ] [ u₁ ]₀ ⟩ v₁ ≡ v₂ ∷ B [ σ₁ ⇑ ] [ u₁ ]₀ →
    Δ ⊩⟨ l [ σ₁ ⇑ ⇑ ] [ u₁ , v₁ ]₁₀ ⟩ t₁ [ σ₁ ⇑ ⇑ ] [ u₁ , v₁ ]₁₀ ≡ t₂ [ σ₂ ⇑ ⇑ ] [ u₂ , v₂ ]₁₀ ∷
      C [ σ₁ ⇑ ⇑ ] [ u₁ , v₁ ]₁₀
  ⊩ᵛ≡∷→⊩ˢ≡∷→⊩≡∷→⊩≡∷→⊩[⇑⇑][]₁₀≡[⇑⇑][]₁₀∷
    {B} {l} {t₁} {t₂} {C} {l′} {l″} t₁≡t₂ σ₁≡σ₂ u₁≡u₂ v₁≡v₂ =
    case wf-∙-⊩ᵛ (wf-⊩ᵛ∷ (wf-⊩ᵛ≡∷ t₁≡t₂ .proj₁)) of λ
      (_ , ⊩B) →
    PE.subst₄ (_ ⊩⟨_⟩_≡_∷_)
      (PE.sym $ doubleSubstComp l _ _ _)
    ...
    ⊩ˢ≡∷∙⇔′ .proj₂
      ( (_ , ⊩B)
      , ( l″
        , PE.subst₂ (_ ⊩⟨_⟩ _ ≡ _ ∷_) (singleSubstComp _ _ l″) (singleSubstComp _ _ B) v₁≡v₂
        )
      , ⊩ˢ≡∷∙⇔′ .proj₂ (wf-∙-⊩ᵛ ⊩B , (l′ , u₁≡u₂) , σ₁≡σ₂)
      )
...
opaque
  ...
  ⊩ᵛ∷→⊩ˢ∷→⊩∷→⊩∷→⊩[⇑⇑][]₁₀∷ :
    ...
    Δ ⊩⟨ l′ [ σ ] ⟩ u ∷ A [ σ ] →
    Δ ⊩⟨ l″ [ σ ⇑ ] [ u ]₀ ⟩ v ∷ B [ σ ⇑ ] [ u ]₀ →
    Δ ⊩⟨ l [ σ ⇑ ⇑ ] [ u , v ]₁₀ ⟩ t [ σ ⇑ ⇑ ] [ u , v ]₁₀ ∷ C [ σ ⇑ ⇑ ] [ u , v ]₁₀
  ⊩ᵛ∷→⊩ˢ∷→⊩∷→⊩∷→⊩[⇑⇑][]₁₀∷ {l′} {l″} ⊩t ⊩σ ⊩u ⊩v =
    ⊩∷⇔⊩≡∷ .proj₂ $
    ⊩ᵛ≡∷→⊩ˢ≡∷→⊩≡∷→⊩≡∷→⊩[⇑⇑][]₁₀≡[⇑⇑][]₁₀∷ {l′ = l′} {l″ = l″} (refl-⊩ᵛ≡∷ ⊩t) (refl-⊩ˢ≡∷ ⊩σ)
      (refl-⊩≡∷ ⊩u) (refl-⊩≡∷ ⊩v)
...
opaque
  ...
  ⊩ᵛ≡→⊩≡∷→⊩≡∷→⊩ˢ≡∷→⊩[]₁₀[]≡[]₁₀[] :
    Γ ∙ A ∙ B ⊩ᵛ⟨ l ⟩ C₁ ≡ C₂ →
    Δ ⊩⟨ l′ [ σ₁ ] ⟩ t₁ [ σ₁ ] ≡ t₂ [ σ₂ ] ∷ A [ σ₁ ] →
    Δ ⊩⟨ l″ [ t₁ ]₀ [ σ₁ ] ⟩ u₁ [ σ₁ ] ≡ u₂ [ σ₂ ] ∷ B [ t₁ ]₀ [ σ₁ ] →
    Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ →
    Δ ⊩⟨ l [ t₁ , u₁ ]₁₀ [ σ₁ ] ⟩ C₁ [ t₁ , u₁ ]₁₀ [ σ₁ ] ≡ C₂ [ t₂ , u₂ ]₁₀ [ σ₂ ]
  ⊩ᵛ≡→⊩≡∷→⊩≡∷→⊩ˢ≡∷→⊩[]₁₀[]≡[]₁₀[]
    {B} {l} {C₁} {C₂} {l′} {l″} C₁≡C₂ t₁[σ₁]≡t₂[σ₂] u₁[σ₁]≡u₂[σ₂] σ₁≡σ₂ =
    PE.subst₃ (_ ⊩⟨_⟩_≡_)
      (PE.sym $ [,]-[]-commute l)
      (PE.sym $ [,]-[]-commute C₁)
      (PE.sym $ [,]-[]-commute C₂) $
    ⊩ᵛ≡→⊩ˢ≡∷→⊩≡∷→⊩≡∷→⊩[⇑⇑][]₁₀≡[⇑⇑][]₁₀ {l′ = l′} {l″ = l″} C₁≡C₂ σ₁≡σ₂ t₁[σ₁]≡t₂[σ₂]
      (PE.subst₂ (_ ⊩⟨_⟩ _ ≡ _ ∷_) (singleSubstLift l″ _) (singleSubstLift B _)
         u₁[σ₁]≡u₂[σ₂])
...
opaque
  ...
  ⊩ᵛ→⊩∷→⊩∷→⊩ˢ∷→⊩[]₁₀[] :
    ...
    Δ ⊩ˢ σ ∷ Γ →
    Δ ⊩⟨ l [ t , u ]₁₀ [ σ ] ⟩ C [ t , u ]₁₀ [ σ ]
  ⊩ᵛ→⊩∷→⊩∷→⊩ˢ∷→⊩[]₁₀[] {l′} {t} {l″} {u} ⊩C ⊩t[σ] ⊩u[σ] ⊩σ =
    ⊩⇔⊩≡ .proj₂ $
    ⊩ᵛ≡→⊩≡∷→⊩≡∷→⊩ˢ≡∷→⊩[]₁₀[]≡[]₁₀[] {l′ = l′} {t₂ = t} {l″ = l″} {u₂ = u} (refl-⊩ᵛ≡ ⊩C)
...
opaque
  ...
  ⊩ᵛ≡∷→⊩≡∷→⊩≡∷→⊩ˢ≡∷→⊩[]₁₀[]≡[]₁₀[]∷ :
    Γ ∙ A ∙ B ⊩ᵛ⟨ l ⟩ t₁ ≡ t₂ ∷ C →
    Δ ⊩⟨ l′ [ σ₁ ] ⟩ u₁ [ σ₁ ] ≡ u₂ [ σ₂ ] ∷ A [ σ₁ ] →
    Δ ⊩⟨ l″ [ u₁ ]₀ [ σ₁ ] ⟩ v₁ [ σ₁ ] ≡ v₂ [ σ₂ ] ∷ B [ u₁ ]₀ [ σ₁ ] →
    Δ ⊩ˢ σ₁ ≡ σ₂ ∷ Γ →
    Δ ⊩⟨ l [ u₁ , v₁ ]₁₀ [ σ₁ ] ⟩ t₁ [ u₁ , v₁ ]₁₀ [ σ₁ ] ≡ t₂ [ u₂ , v₂ ]₁₀ [ σ₂ ] ∷
      C [ u₁ , v₁ ]₁₀ [ σ₁ ]
  ⊩ᵛ≡∷→⊩≡∷→⊩≡∷→⊩ˢ≡∷→⊩[]₁₀[]≡[]₁₀[]∷
    {B} {l} {t₁} {t₂} {C} {l′} {l″} t₁≡t₂ u₁[σ₁]≡u₂[σ₂] v₁[σ₁]≡v₂[σ₂] σ₁≡σ₂ =
    PE.subst₄ (_ ⊩⟨_⟩_≡_∷_) (PE.sym $ [,]-[]-commute l) (PE.sym $ [,]-[]-commute t₁)
      (PE.sym $ [,]-[]-commute t₂) (PE.sym $ [,]-[]-commute C) $
    ⊩ᵛ≡∷→⊩ˢ≡∷→⊩≡∷→⊩≡∷→⊩[⇑⇑][]₁₀≡[⇑⇑][]₁₀∷ {l′ = l′} {l″ = l″} t₁≡t₂ σ₁≡σ₂ u₁[σ₁]≡u₂[σ₂]
      (PE.subst₂ (_ ⊩⟨_⟩ _ ≡ _ ∷_) (singleSubstLift l″ _) (singleSubstLift B _)
         v₁[σ₁]≡v₂[σ₂])
...
opaque
  ...
  ⊩ᵛ∷→⊩∷→⊩∷→⊩ˢ∷→⊩[]₁₀[]∷ :
    ...
    Δ ⊩⟨ l″ [ u ]₀ [ σ ] ⟩ v [ σ ] ∷ B [ u ]₀ [ σ ] →
    Δ ⊩ˢ σ ∷ Γ →
    Δ ⊩⟨ l [ u , v ]₁₀ [ σ ] ⟩ t [ u , v ]₁₀ [ σ ] ∷ C [ u , v ]₁₀ [ σ ]
  ⊩ᵛ∷→⊩∷→⊩∷→⊩ˢ∷→⊩[]₁₀[]∷ {l′} {u} {l″} {v} ⊩t ⊩u[σ] ⊩v[σ] ⊩σ =
    ⊩∷⇔⊩≡∷ .proj₂ $
    ⊩ᵛ≡∷→⊩≡∷→⊩≡∷→⊩ˢ≡∷→⊩[]₁₀[]≡[]₁₀[]∷ {l′ = l′} {u₂ = u} {l″ = l″} {v₂ = v} (refl-⊩ᵛ≡∷ ⊩t)
