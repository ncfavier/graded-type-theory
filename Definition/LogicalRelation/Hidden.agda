...
opaque
  ...
  ⊩∷ne⇔ {A} A-ne =
    ...
    lemma = <ᵘ-rec _ λ where
      l rec (noemb (ne _ _ A⇒*A′ _ _)) (neₜ u t⇒*u (neNfₜ inc u-ne u~u)) →
        case whnfRed* A⇒*A′ (ne A-ne) of λ {
          PE.refl → inc , u , t⇒*u , u-ne , u~u }
...
opaque
  ...
  ⊩ne≡⇔ {A} {B} A-ne =
    ...
    lemma :
      ...
      Neutrals-included × ∃ λ C → Neutral C × Γ ⊢ B ⇒* C × Γ ⊢ A ≅ C
    lemma = <ᵘ-rec _ λ where
      l rec (noemb (ne _ _ A⇒*A′ _ _)) (ne₌ inc C B⇒*C C-ne A′≅C) →
        case whnfRed* A⇒*A′ (ne A-ne) of λ {
          PE.refl →
        inc , C , C-ne , B⇒*C , A′≅C }
...
opaque
  ...
  ⊩≡∷ne⇔ {A} {l} A-ne =
    ...
    lemma :
      ...
      Γ ⊩neNf u₁ ≡ u₂ ∷ A
    lemma = <ᵘ-rec _ λ where
      l rec (noemb (ne _ _ A⇒*A′ _ _)) (neₜ₌ u₁ u₂ t₁⇒*u₁ t₂⇒*u₂ u₁≡u₂) →
        case whnfRed* A⇒*A′ (ne A-ne) of λ {
