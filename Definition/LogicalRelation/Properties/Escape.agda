...
escapeTerm (Emptyᵣ D) (Emptyₜ _ d _ _) =
  conv (redFirst*Term d) (sym (subset* D))
escapeTerm (Unitᵣ′ _ _ _ D _) (Unitₜ _ d _ _) =
  conv (redFirst*Term d) (sym (subset* D))
...
escapeEq (Unitᵣ′ _ (Levelₜ m d _ _) _ D ok) (Unit₌ _ D′ k≡k′) =
...
escapeTermEq (Emptyᵣ D) (Emptyₜ₌ k k′ d d′ k≡k′ prop) =
  ...
  in  ≅ₜ-red (D , Emptyₙ) (d , ne natK) (d′ , ne natK′)
        k≡k′
escapeTermEq (Unitᵣ′ k [k] k< D ok) (Unitₜ₌ˢ ⊢t ⊢u η) =
  let t≅u = ≅ₜ-η-unit (escapeLevel [k]) ⊢t ⊢u ok η
      A≡Unit = subset* D
  in ≅-conv t≅u (sym A≡Unit)
escapeTermEq (Unitᵣ′ _ _ _ D _) (Unitₜ₌ʷ _ _ d d′ k≡k′ prop _) =
