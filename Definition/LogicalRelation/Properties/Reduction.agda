...
redSubst*Term t⇒u (Uᵣ′ l [l] ≤ᵘ-refl D) (Uₜ A d typeA A≡A [u]) =
  let A≡K  = subset* D
      d′ = conv* t⇒u A≡K ⇨∷* d
      [t] , t≡u = redSubst* (univ* (conv* t⇒u A≡K)) [u]
  in Uₜ A d′ typeA A≡A [t] ,
...
redSubst*Term t⇒u (Unitᵣ {s} (Unitᵣ k [k] k≤ D _)) (Unitₜ n d n≡n prop) =
  let t⇒u         = conv* t⇒u (subset* D)
...
opaque
  ...
  redSubst*Term′
    t⇒*u (Unitᵣ {s} (Unitᵣ k [k] k≤ A⇒*Unit _)) (Unitₜ v t⇒*v v≅v v-ok) =
