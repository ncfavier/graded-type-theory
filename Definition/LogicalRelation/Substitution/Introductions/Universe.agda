...
opaque
  ...
  ⊩U⇔ =
      lemma _ ∘→ U-elim
    , λ ([t] , t<l) → Uᵣ′ _ [t] t<l (id (Uⱼ (escapeLevel [t])))
    where
    lemma :
      ∀ l → Γ ⊩⟨ l ⟩U U t →
      Σ (Γ ⊩Level t ∷Level) λ [t] → reflect-level [t] <ᵘ l
    lemma = <ᵘ-rec _ λ where
      l rec (noemb (Uᵣ k [k] k<l U⇒*U)) →
        case U⇒*U→≡ U⇒*U of λ {
          PE.refl → [k] , k<l }
...
opaque
  ...
  ⊩∷U⇔ =
    ...
    , (λ ([t] , t<l , ⊩A , _ , A⇒*B , B-type , B≅B) →
          Uᵣ′ _ [t] t<l (id (Uⱼ (escapeLevel [t])))
...
opaque
  ...
  Type→⊩∷U⇔ {A} {Γ} {l} {t} A-type =
    ...
    (∃ λ [t] → reflect-level [t] <ᵘ l × (Γ ⊩⟨ reflect-level [t] ⟩ A) × Γ ⊢≅ A ∷ U t) □⇔
...
opaque
  ...
  ⊩ᵛU : Γ ⊩ᵛ⟨ l ⟩ t ∷ Level → Γ ⊩ᵛ⟨ ω+0 ⟩ U t
  ⊩ᵛU {Γ} {t} ⊩t =
    ⊩ᵛ⇔ʰ .proj₂
      ( wf-⊩ᵛ (wf-⊩ᵛ∷ ⊩t)
      , λ {_} {Δ} {σ₁} {σ₂} →
          λ σ₁≡σ₂ →
            let (⊩t[σ₁] , ⊩t[σ₂] , ⊩t≡) = ⊩≡∷Level⇔ .proj₁ (⊩ᵛ∷⇔ʰ .proj₁ ⊩t .proj₂ σ₁≡σ₂)
                ⊢Δ = escape-⊩ˢ≡∷ σ₁≡σ₂ .proj₁
            in
              ⊩U≡⇔ .proj₂ $
                ⊩t[σ₁]
...
opaque
  ...
  ⊩ᵛU∷U : Γ ⊩ᵛ⟨ l ⟩ t ∷ Level → Γ ⊩ᵛ⟨ ω+0 ⟩ U t ∷ U (sucᵘ t)
  ⊩ᵛU∷U {Γ} {t} ⊩t =
    ⊩ᵛ∷⇔ʰ .proj₂
      ( ⊩ᵛU (sucᵘᵛ ⊩t)
      , λ {_} {Δ} {σ₁} {σ₂} σ₁≡σ₂ →
...
opaque
  ...
  ⊩ᵛ≡∷U→⊩ᵛ≡ A≡B∷U =
    ...
    ⊩ᵛ≡⇔ʰ .proj₂
      ( wf-⊩ᵛ ⊩U
      , λ σ₁≡σ₂ →
        let ([t] , t<l , A≡A , _) = ⊩≡∷U⇔ .proj₁ (A≡B∷U σ₁≡σ₂) in
        emb-⊩≡ (<ᵘ→≤ᵘ <ᵘ-ω) A≡A
      )
